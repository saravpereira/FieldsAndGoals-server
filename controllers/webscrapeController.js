...
async function scrapeEspn(startDate, endDate, res) {
  try {
    ...
    await page.setUserAgent(
      'Mozilla/5.0 (iPhone; CPU iPhone OS 7_4_0; like Mac OS X) AppleWebKit/534.49 (KHTML, like Gecko)  Chrome/53.0.1780.199 Mobile Safari/533.3'
    );
    ...
    while (current <= end) {
      ...
      const matchData = await page.evaluate((date) => {
        ...
        matchElements.forEach((container) => {
          ...
          matchInfo.matchDate = matchDateObj.toLocaleDateString(
            'en-US',
            options
          );

          const leagueElement = container
            .closest('.Card')
          ...
          const homeScoreElement = container.querySelector(
            '.ScoreboardScoreCell__Item--home .ScoreCell__Score'
          );
          matchInfo.homeScore = homeScoreElement?.textContent?.trim() ?? ' ';

          const awayScoreElement = container.querySelector(
            '.ScoreboardScoreCell__Item--away .ScoreCell__Score'
          );
          matchInfo.awayScore = awayScoreElement?.textContent?.trim() ?? ' ';

          const matchStatusElement =
            container.querySelector('.ScoreCell__Time');
          matchInfo.matchStatus = matchStatusElement?.textContent?.trim() ?? '';

          const homeLogoElement = container.querySelector(
            '.ScoreboardScoreCell__Item--home .ScoreboardScoreCell__Logo'
          );
          matchInfo.homeLogo = homeLogoElement?.getAttribute('src') ?? '';

          const awayLogoElement = container.querySelector(
            '.ScoreboardScoreCell__Item--away .ScoreboardScoreCell__Logo'
          );
